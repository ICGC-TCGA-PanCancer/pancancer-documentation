...
while (my $line = <FH>) {
  ...
  my @cols = split "," , $line;
  #print "$cols[4],";
  push (@spec_marc1,$cols[4]);
}
close(FH);

open(FH,"train_2.csv") or die("Can't open guestbook.txt: $!");
while (my $line = <FH>) {
  chomp $line;
  my @cols = split "," , $line;
  #print "$cols[4],";
  push (@spec_marc2,$cols[4]);
}
close(FH);
...
# does it NOT have "workflow" in it?
        ...
        if ($check == 1){
        if ($check_all == 1){$data_all += 1;}
        #       print "$spec,";
                #print "$line_num $line\n";
                $data1 += 1;}
        }
        elsif($check_all == 1){$data_all += 1;}
}
...
print qq({"repo": $elem, "align": $change, "total_unalign": $all_unalign, "remaining": $all_remain}\n);}
}
