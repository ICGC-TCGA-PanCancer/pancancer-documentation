...
worker nodes by increasing the number of nodes in the .cfg file.  We typically 
...
Now that you have customized the settings in .cfg file, the
next step is to launch a computational node. Note, each cluster gets its own target directory 
which you can specify the name of in .cfg file when you make a cluster block
; you cannot resuse these.  Within the
target dir you will find a log for each node (simply master.log for a
...
You can follow the progress of this cluster launch in another terminal with the following command.
